...
async function processEPUB(target) {
	...
	epub.on("end", async function () {
		...
		console.log(`Upload ${title} complete!`);
	});
	epub.parse();

	async function coverPage(subroot) {
		const token = authenticate("hdagnew@ucdavis.edu", subdomain);
...
async function photoTest() {
	...
	if (response.ok)
		console.log(`https://${subdomain}.libretexts.org/${path}`);
}
