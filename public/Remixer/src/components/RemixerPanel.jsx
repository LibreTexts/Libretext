...
export default class RemixerPanel extends React.Component {
  ...
  async componentDidMount() {
    this.setState({ LibraryTree: await RemixerPanel.getSubpages('home', this.props.subdomain, false, true) });
...
//					data.dataTransfer.setDragImage($("<div>hurz</div>").appendTo("body")[0], -10, -10);
      ...
      icon: (event, data) => {
        if (data.node.getLevel() === 1)
          return `https://libretexts.org/img/LibreTexts/glyphs/${this.props.subdomain}.png`;
      },
    });
    LTRight.fancytree({
  ...
  render() {
    ...
    return <div id='LTForm'>
      <div className="LTFormHeader">
        ...
        <button onClick={() => this.dialog.dialog('open')}>Start Over</button>
      </div>
      <div id='LTFormContainer'>
        <div>Library Panel<select id='LTFormSubdomain'
                                  onChange={this.setSubdomain}
                                  value={this.props.subdomain}>{this.getSelectOptions()}</select>
          <div id='LTLeft'></div>
        </div>
        <div>Remix Panel
