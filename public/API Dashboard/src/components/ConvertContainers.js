...
export default class ConvertContainers extends React.Component {
	constructor() {
		super();
		this.state = {
			root: "",
	...
	componentDidMount() {
		this.socket = io(this.props.devMode ? 'https://computer.miniland1333.com/' : 'https://api.libretexts.org/', {path: '/bot/ws'});
	...
	verifyRequest() {
		...
		if (!request.root) {
			...
			if (confirm(`The bot will find all old containers. No Changes will be made.`))
				this.sendRequest(request);
		}
		else if (confirm(`The bot will upgrade all old containers. Click OK to proceed.`)) {
			this.sendRequest(request);
		}
	}
	...
	sendRequest(request) {
		...
		this.socket.emit('convertContainers', request);
	}
		...
		switch (this.state.status) {
			...
			case 'processing':
				return <div className="status" style={{backgroundColor: 'orange'}}>
					<div>
						Old container{this.state.findOnly ? ' Search' : ' Upgrade'} In Progress
