class simpleGUI {
	static display() {
		...
		target.innerHTML = `<div>Source Root URL: <input id="copySource" oninput="simpleGUI.reset()" value="${originalURL}" placeholder="Paste source address here"/></div><div>Destination Root URL: <input id="copyDestination" oninput="simpleGUI.reset()" placeholder="Paste destination address here"/></div><button id="copyVerify" onclick="simpleGUI.verify()">Verify</button><div id="copyOutput"></div><div id="copyResults"></div><div id="copyErrors"></div>`;
...
<button id="copyExecute" onclick="simpleGUI.copy()">Copy to Destination Root</button>`;
		...
		async function isCoverpage(origin, path) {
			...
			let tags = await fetch(origin + "/@api/deki/pages/=" + encodeURIComponent(encodeURIComponent(path)) + "/tags?dream.out.format=json", {headers: await simpleGUI.getHeaders(origin)});
			tags = await
				tags.json();
			if (tags.tag) {
				if (tags.tag.length) {
		...
		async function getTree(origin, path) {
			let tree = await fetch(origin + "/@api/deki/pages/=" + encodeURIComponent(encodeURIComponent(path)) + "/subpages?dream.out.format=json", {headers: await simpleGUI.getHeaders(origin)});
			...
			async function subpageCallback(info, isRoot) {
				...
				async function subpage(subpage, index) {
					...
					if (hasChildren) { //recurse down
						children = await
							fetch(origin + "/@api/deki/pages/=" + encodeURIComponent(encodeURIComponent(path)) + "/subpages?dream.out.format=json", {headers: await simpleGUI.getHeaders(origin)});
	...
	static async copy() {
		...
		async function doCopy(destRoot, tree) {
			...
			for (let i = 0; i < tree.length; i++) {
				...
				//get info
				let info = fetch(instance.Sorigin + "/@api/deki/pages/=" + encodeURIComponent(encodeURIComponent(child.path)) + "/info?dream.out.format=json", {headers: await simpleGUI.getHeaders(instance.Sorigin)});
				
				//get Tags
				let copyContent = false;
				let tags = await fetch(instance.Sorigin + "/@api/deki/pages/=" + encodeURIComponent(encodeURIComponent(child.path)) + "/tags?dream.out.format=json", {headers: await simpleGUI.getHeaders(instance.Sorigin)});
				...
				if (copyContent) {
					content = await fetch(instance.Sorigin + "/@api/deki/pages/=" + encodeURIComponent(encodeURIComponent(child.path)) + "/contents?mode=raw", {headers: await simpleGUI.getHeaders(instance.Sorigin)});
...
template('TranscludeAutoNumTitle');</pre>`;
				...
				let response = await fetch(instance.Dorigin + "/@api/deki/pages/=" + encodeURIComponent(encodeURIComponent(path + child.relativePath)) + "/contents?abort=exists", {
					method: "POST",
					body: content, headers: await simpleGUI.getHeaders(instance.Dorigin)
				});
				...
				switch (response.status) {
					...
					case 200:
						...
						fetch(instance.Dorigin + "/@api/deki/pages/=" + encodeURIComponent(encodeURIComponent(path + child.relativePath)) + "/tags", {
							method: "PUT",
							body: tags,
							headers: await simpleGUI.getHeaders(instance.Dorigin, {"Content-Type": "text/xml; charset=utf-8"})
						}).then();
						
						fetch(instance.Sorigin + "/@api/deki/pages/=" + encodeURIComponent(encodeURIComponent(child.path)) + "/properties?dream.out.format=json", {headers: await simpleGUI.getHeaders(instance.Sorigin)}).then(async (response) => {
									...
									case "mindtouch.idf#subpageListing":
										if (tags.includes("article:topic-category")) {
											fetch(instance.Dorigin + "/@api/deki/pages/=" + encodeURIComponent(encodeURIComponent(path + child.relativePath)) + "/properties", {
												...
												headers: await simpleGUI.getHeaders(instance.Dorigin,{"Slug": content[i].name})
											}).then();
										}
										break;
									//subpageListing check
						...
						if (info.title !== endPath) {
							fetch(instance.Dorigin + "/@api/deki/pages/=" + encodeURIComponent(encodeURIComponent(path + child.relativePath)) + "/move?title=" + info.title + "&name=" + path + child.relativePath, {
								method: "POST",
								headers: await simpleGUI.getHeaders(instance.Dorigin)
							}).then();
						}
						
						//Thumbnail
						fetch(instance.Sorigin + "/@api/deki/pages/=" + encodeURIComponent(encodeURIComponent(child.path)) + "/files/=mindtouch.page%2523thumbnail", {headers: await simpleGUI.getHeaders(instance.Sorigin)}).then(async (response) => {
							if (response.ok) {
