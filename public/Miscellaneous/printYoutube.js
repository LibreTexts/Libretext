		...
		for (let i = 0; i < candidates.length; i++) {
			...
			if (iframe && iframe.src && !iframe.classList.contains("youtubeIFrame")) {
				...
				if (videoID != null) {
					...
					youtubeImg.src = "https://img.youtube.com/vi/" + videoID + "/0.jpg";
					youtubeImg.classList.add("youtubeImage");
					$(youtubeImg).insertAfter(iframe);
					...
					$(youtubeImg).insertAfter(iframe);
					$(extraDiv).insertAfter(youtubeImg);
				}
			}
		}
	});
