...
window['GL' + index].rotate = function (spinDelta = 0) {
    //initial rotation
    let dx = window['GL' + index]?.initialRotate?.x || 0;
    let dy = window['GL' + index]?.initialRotate?.y || 0;
    let dz = window['GL' + index]?.initialRotate?.z || 0;
...
var step = function (timestamp, index) {
    if (window['GL' + index].change) {
        ...
        window['GL' + index].rotate(window['GL' + index].dx % 180);
...
function download(query, index) {
    ...
    $.get(uri, function (ret) {
        ...
        if (currentScript.dataset.initialRotation) {
            let [x = 0, y = 0, z = 0] = currentScript.dataset.initialRotation.split(',');
            window['GL' + index].initialRotate = {
                x: parseFloat(x) / 180,
                y: parseFloat(y) / 180,
                z: parseFloat(z) / 180
            }
            window['GL' + index].rotate();
        }
