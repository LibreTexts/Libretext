...
// this library is organized into the following sections:
		...
		if (applet._cacheFiles && Jmol._fileCache && !fileName.endsWith(".js")) {
			...
			if (data) {
				...
				fSuccess = function (fileName, data) {
					fSuccess(Jmol._fileCache[fileName] = data)
				};
			}
		}
		return fSuccess;
	...
	Jmol._playAudio = function (applet, params) {
		...
		var get = (params.get ? function (key) {
			return params.get(key)
		} : null);
		var put = (params.put ? function (key, val) {
		...
		if (jmolAudio) {
			...
			jmolAudio.myClip = {
				...
				setMicrosecondPosition: function (us) {
					audio.currentTime = us / 1e6;
				}
			};
		}
		audio.controls = "true";
	...
	Jmol._loadFileData = function (applet, fileName, fSuccess, fError) {
		...
		var info = {
			...
			success: function (a) {
				Jmol._loadSuccess(a, fSuccess)
			},
			error: function () {
				Jmol._loadError(fError)
			}
		}
	...
	Jmol._getInfoFromDatabase = function (applet, database, query) {
		if (database == "====") {
			...
			var info = {
				...
				success: function (data) {
					Jmol._ajaxDone();
					Jmol._extractInfoFromRCSB(applet, database, query, data)
				},
				error: function () {
					Jmol._loadError(null)
				},
				async: Jmol._asynchronous
			}
		...
		query = "?call=getInfoFromDatabase&database=" + database
			+ "&query=" + encodeURIComponent(query);
		return Jmol._contactServer(query, function (data) {
			Jmol._setInfo(applet, database, data)
		});
	}
	...
	Jmol._extractInfoFromRCSB = function (applet, database, query, output) {
		...
		if (query.length == 4 && n != 1) {
			...
			Jmol._loadFileData(applet, url, function (data) {
				Jmol._setInfo(applet, database, data)
			});
		}
	}
		...
		if (isHttps2Http || asBase64 || !isMyHost && !isDirectCall || !fSuccess && cantDoSynchronousLoad) {
			...
			if (fSuccess) {
				info.success = function (data) {
					fSuccess(Jmol._xhrReturn(info.xhr))
				};
				info.error = function () {
	...
	Jmol._loadFileAsynchronously = function (fileLoadThread, applet, fileName, appData) {
		if (fileName && fileName.indexOf("?") != 0) {
			...
			var fSuccess = function (data) {
				Jmol._setData(fileLoadThread, fileName, fileName0, data, appData, applet)
			};
			fSuccess = Jmol._checkCache(applet, fileName, fSuccess);
...
*/
	...
	Jmol._setDestroy = function (applet) {
		...
		if (Jmol.featureDetection.allowDestroy)
			Jmol.$windowOn('beforeunload', function () {
				Jmol._destroy(applet);
			});
	}
...
//			if (app._viewType == "JSV" && !app._currentView.JMOL)
	...
	Jmol._jsSetMouse = function (canvas) {
		...
		var doIgnore = function (ev) {
			return (!ev.target || ("" + ev.target.className).indexOf("swingjs-ui") >= 0)
		};
		...
		Jmol.$bind(canvas, "contextmenu", function () {
			return false;
		});
...
//    delete Dialog.htDialogs[btns[i].id]
		...
		Swing.setDialog = function (dialog) {
			...
			Jmol.$bind("#" + dialog.id + " .JButton", "mousedown touchstart", function (event) {
				jd.ignoreMouse = true
			});
			Jmol.$bind("#" + dialog.id + " .JComboBox", "mousedown touchstart", function (event) {
				jd.ignoreMouse = true
			});
			Jmol.$bind("#" + dialog.id + " .JCheckBox", "mousedown touchstart", function (event) {
				jd.ignoreMouse = true
			});
			Jmol.$bind("#" + dialog.id + " .JTextField", "mousedown touchstart", function (event) {
				jd.ignoreMouse = true
			});
			Jmol.$bind("#" + dialog.id + " .JTable", "mousedown touchstart", function (event) {
				jd.ignoreMouse = true
			});
			Jmol.$bind("#" + dialog.id + " .JScrollPane", "mousedown touchstart", function (event) {
				jd.ignoreMouse = true
			});
			Jmol.$bind("#" + dialog.id + " .JEditorPane", "mousedown touchstart", function (event) {
				jd.ignoreMouse = true
			});
...
// methods called from other modules have no "_" in their name
		...
		View.updateFromSync = function (applet, msg) {
			...
			var A = ((id = Jmol._getAttr(msg, "atoms")) && msg.indexOf("selectionhalos ON") >= 0
				? eval("[" + id + "]") : []);
			setTimeout(function () {
				if (applet._currentView == view) View.updateAtomPick(applet, A)
			}, 10);
			if (Jmol.User.viewUpdatedCallback)
...
//////// swingjs.api.HTML5Applet interface
		...
		proto._getContentLayer = function () {
			return Jmol.$(this, "contentLayer")[0]
		};
		proto._repaintNow = function () {
			Jmol._repaint(this, false)
		};
////////
		...
		proto._createCanvas2d = function (doReplace) {
			...
			if (this._isLayered) {
				...
				Jmol._$(img.id).css({
					zIndex: Jmol._getZ(this, "image"), position: "absolute", left: "0px", top: "0px",
		...
		proto._resize = function () {
			...
			if (Jmol[s])
				clearTimeout(Jmol[s]);
			var me = this;
			Jmol[s] = setTimeout(function () {
				Jmol._repaint(me, true);
				Jmol[s] = null
			}, 100);
		}
			...
			if (image == null) {
				...
				if (bytes == null) {
					image.onload = function () {
						Jmol._loadImage(platform, echoName, path, null, fOnload, image)
					};
					image.src = path;
					return null;
				}
...
//				break;
	...
	*/
		...
		proto._restoreState = function (clazzName, state) {
			...
			switch (state) {
				case "setOptions":
					return function (_setOptions) {
						applet.__startAppletJS(applet)
					};
				case "render":
					return function () {
						setTimeout(function () {
		...
		proto._loadFile = function (fileName, script, chemID, _jmol_loadFile) {
			...
			if (!this._scriptLoad(fileName, script)) {
				...
				Jmol._loadFileData(this, fileName,
					function (data) {
						me.__loadModel(data, script, chemID)
					},
					function () {
						me.__loadModel(null)
					}
...
*/
			...
			addProto(proto, "getClass", function () {
				return Clazz.getClass(this);
			});
			
			addProto(proto, "clone", function () {
 ...
 */
			...
			addProto(proto, "toString", function () {
			...
			switch (typeof obj) {
				...
				case "function":
					if (obj.__CLASS_NAME__)
						return (arguments[1] ? obj.__CLASS_NAME__ : "Class");
		...
		;(function () {
			...
			try {
				...
				if (/Opera[\/\s](\d+\.\d+)/.test(navigator.userAgent)) {// opera throws an exception with fixed messages like "Statement on line 23: Cannot convert undefined or null to Object Backtrace: Line....long text... "
					...
					_isNPEExceptionPredicate = function (e) {
						return e.message.indexOf(_NPEMsgFragment) != -1;
					};
					...
					_isNPEExceptionPredicate = function (e) {
						return _exceptionNPERegExp.test(e.message);
					};
 ...
 */
	 ...
	 */
				...
				if (mode == 0 && isIndex && !map["@java"] && pkg.indexOf("java") != 0 && needPackage("java")) {
					_Loader.loadPackage("java", fSuccess ? function (_package) {
						_Loader.loadPackageClasspath(pkg, base, isIndex, fSuccess, 1)
					} : null);
					if (fSuccess)
						return;
				}
				if (pkg instanceof Array) {
					...
					if (fSuccess) {
						if (pt < pkg.length)
							_Loader.loadPackageClasspath(pkg[pt], base, isIndex, function (_loadPackageClassPath) {
								_Loader.loadPackageClasspath(pkg, base, isIndex, fSuccess, 1, pt + 1)
							}, 1);
						else
							fSuccess();
					} else {
...
//	}
				...
				if (!needBeingQueued) { // can be loaded directly
					...
					loadScript(n, n.path, n.requiredBy, false, onLoaded ? function (_loadClass) {
						isLoadingEntryClass = bSave;
						onLoaded()
					} : null);
				}
			};
...
//      System.out.println("firing in loadScript " + file + " " + (fSuccess && fSuccess.toString()))
			...
			var W3CScriptOnCallback = function (path, forError, fSuccess) {
				...
				return function () {
					...
					var f;
					if (fSuccess)
						f = function (_W3scriptFS) {
							removeScriptNode(node);
							tryToLoadNext(path, fSuccess);
						};
					else
						f = function (_W3script) {
							removeScriptNode(node);
							tryToLoadNext(path)
						};
					if (loadingTimeLag >= 0)
...
Jmol.___JmolDate = "$Date: 2018-03-13 12:01:07 -0500 (Tue, 13 Mar 2018) $"
Jmol.___fullJmolProperties = "src/org/jmol/viewer/Jmol.properties"
Jmol.___JmolVersion = "14.29.10"
