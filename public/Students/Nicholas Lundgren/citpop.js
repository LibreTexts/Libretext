...
// Consolidate, Transform, and Output Citation Info //
function nikGetCite(verbose=false){
    /* Information Consolidation */
    // Access Date
    var today = new Date();
    var accdate = [today.getFullYear(),today.getMonth()+1,today.getDate()];

    // Resource Name
    var fullbook = $("meta[property='og:title']").prop('content');
    var book = fullbook.slice(fullbook.indexOf(':')+2,fullbook.lastIndexOf('(')-1);
    ...
    // Authors
    if($("li[class='mt-author-information']").find('a').text()!= ''){
        var fullauthors = $("li[class='mt-author-information']").find('a').text()
        var authors = namesplitter(fullauthors)
        jsobj['author'] = authors
    }
...
// Author Name Helper Function (names are hard to split)//  
function namesplitter(name, verbose=false){
    var rawnames = name;                
    var splind = [0];               
    var spls = [' and ', '& ', ', '];
    var  rawauthnames = new Array; 
    var authors = new Array;       
    for (i=0; i < spls.length; i++){ 
        var finder = spls[i];
        var n=0;
        while(n < rawnames.length){
                var sfield = rawnames.slice(n)
                if((sfield.search(finder) != -1)){
                    var x = n+sfield.indexOf(finder)+1;
                    var y = x + finder.length - 1;
                    splind.push(x,y);
                    n= y;
                }else {
                    n = rawnames.length;
    ...
    for (i=0; i<rawauthnames.length; i++){ // Parses names into family and given names
        var rawname = rawauthnames[i];
        var namedict = {};
        if (rawname.search('\\.') != -1){
            var fname = rawname.slice(0,rawname.lastIndexOf('.')+1);
            var lname = rawname.slice(rawname.lastIndexOf('.')+2,rawname.length);
            namedict = {
                given: fname,
                family: lname
            }}else{
            var fname ='';
            var lname ='';
            var arname = rawname.split(' ');
            var lnameind = arname.length - 1;
...
/*  citeInt is the initial hook that everything grabs
    citeBg is a black layover of the original page. Upon clicking this
	the modal is closed
    citeCont contains citeText (the textbox area), select and close (the buttons)
	and Head which is just a title. */
    var citeInt = document.createElement("div");
    var citeBg = document.createElement("div");
    var citeCont = document.createElement("div");
    var citeHead = document.createElement("div");
    var citeText = document.createElement("TEXTAREA");
    var citeButtons = document.createElement("div");
    var citeSelect = document.createElement("SELECT");
    ...
    // Attach the elements to each other //
    ...
    for(i=0;i<Object.keys(elements).length;i++){
        Object.values(elements)[i].setAttribute("id",Object.keys(elements)[i]);
    };
    for(i=0;i<Object.keys(options).length;i++){
        citeSelect[i] = new Option(Object.values(options)[i],Object.keys(options)[i])
        if(i==0){
            citeSelect[i].setAttribute('selected',true)
            citeSelect[i].setAttribute('disabled',true)
        }
    };

    // Add functionality to the relevant elements //
    $('#citeHead').append('<h1>Citation Generator</h1>');
    $('#citeClose').append('Close');
    $('#citeText').text('Please Choose a Format.');
    $('#citeSelect').on('change',function(){nikGetCite()})
    $('#citeSelect').val('apa')
    nikGetCite()
