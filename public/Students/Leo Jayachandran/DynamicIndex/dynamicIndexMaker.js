...
function trimTermTag(termList/*Array with Terms*/) {
    ...
    for (var i = 0; i < newList.length; i++) {
        for (var u = 0; u < testStrings.length; u++) {
...
function sortPage(pageList/*Array with Pages*/) {
    ...
    newList.sort(function (a, b) {return ((a.pageName.toLowerCase() > b.pageName.toLowerCase()) ? 1 : -1)});
    return newList;
}
function sortTerms(termList/*Array with Pages*/) {
    let newList = termList.slice();
    newList.sort(function (a, b) {return ((a.name.toLowerCase() > b.name.toLowerCase()) ? 1 : -1)});
    return newList;
}

async function getIndexingURL () {
    var results;
    var coverPage = await LibreTexts.getCoverpage();
...
function jsonToHTMLTable (inputPageList){
    ...
    for (var i = 0; i < alphabet.length; i++) {
        ...
        for (var u = termPos; u < pageList.taggedTerms.length; u++) {
    ...
    var $indexLetterList = $("#indexLetterList");
    var $indexTable = $("#indexTable").html("");
    //Terms sorted by alphabet
    for (var i = 0; i < alphabetisedIndex.length; i++) {
        ...
        var $indexBodyRow = $(`<div class = 'indexBodyRows' id = 'indexRow${alphabetisedIndex[i].letter}'></div>`).appendTo($letterEntry);
...
async function processContents(){
    ...
    var dynamicURL = await getIndexingURL();
